...
export function ModelUploadZone({ onUpload, currentImage, onRemove }: ModelUploadZoneProps) {
  ...
  return (
    <div className="space-y-4">
      {currentImage ? (
        <div className="relative group rounded-xl overflow-hidden border border-white/20 bg-black aspect-[3/4] max-w-[200px] mx-auto">
          ...
          {isUploading ? (
            ...
            <>
              <div className="size-16 rounded-2xl flex items-center justify-center">
                <User className={cn("size-7", isDragActive ? "text-white/60" : "text-white/30")} />
              </div>
              <div>
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-4">
        <p className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-2">Tips for best results</p>
        <ul className="space-y-1.5">
          {TIPS.map((tip) => (
            <li key={tip} className="flex items-start gap-2 text-xs text-white/50">
              <CheckCircle2 className="size-3.5 mt-0.5 shrink-0 text-white/30" />
              {tip}
            </li>
          ))}
        </ul>
